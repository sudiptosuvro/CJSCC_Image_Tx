...
class UnifiedCAM(nn.Module):
    def __init__(self, csi_dim: int, channels: int, hidden: int = 96, reduction: int = 4):
        super().__init__()
        ...
        nn.init.zeros_(self.csi_mlp[-1].weight)
        nn.init.zeros_(self.csi_mlp[-1].bias)
    ...
    def forward(self, x, csi):
        ...
        feat_attn = self.feature_mlp(self.pool(x))

        gamma, beta = self.csi_mlp(csi).chunk(2, dim=1)
        gamma = gamma.view(B, C, 1, 1)
        beta = beta.view(B, C, 1, 1)

        cam = torch.sigmoid(feat_attn * (1.0 + gamma) + beta)

        return x * (1.0 + 0.3 * torch.sigmoid(self.scale) * (cam - 0.5))
...
class LCDG(nn.Module):
    ...
    def __init__(self, cfg: LCDGConfig) -> None:
        ...
        self.mask_scale = nn.Parameter(torch.tensor(-3.0))
    ...
    def _mask_survival_prior(self, rho: torch.Tensor,
        ...
        rho_gate = torch.sigmoid(-(rho_v - 0.25) / 0.03)
        return prior * snr_gate * rho_gate


    def forward(self, x: torch.Tensor, csi: torch.Tensor,
                is_decoder: bool = False) -> torch.Tensor:
        B, C, H, W = x.shape
        G     = self.cfg.groups
        gsize = C // G
        ...
        rho = torch.where(
            rho_raw > 1e-7,
            rho_raw.clamp(1e-6, 1.0),
            torch.full_like(rho_raw, 1.0 / 12.0)
        )

        try:
            emb = self.csi_embedder(csi)
        except Exception:
            emb = csi[:, :self.cfg.emb_dim]

        avg_map = x.mean(dim=1, keepdim=True)
        max_map, _ = x.max(dim=1, keepdim=True)
        S_s_static = torch.sigmoid(
            self.spatial_gate(torch.cat([avg_map, max_map], dim=1))
        )
        ...
        x_sc = self.unified_cam(x_sc, csi)

        mask_prior = self._mask_survival_prior(rho, snr_norm, C)
        m_scale = 0.10 * torch.sigmoid(self.mask_scale)
...
class PassThrough(nn.Module):
    def forward(self, x: torch.Tensor, csi: torch.Tensor = None,
                is_decoder: bool = False) -> torch.Tensor:
        return x
...
class NullCSI_LCDG(nn.Module):
    ...
    def forward(self, x: torch.Tensor, csi: torch.Tensor,
                is_decoder: bool = False) -> torch.Tensor:
        return self.lcdg(x, torch.zeros_like(csi), is_decoder=is_decoder)