...
def build_csi_from_snr_rho_channel(
    ...
    rho_norm   = torch.log(rho_tensor / (1.0 / 12.0) + 1e-8)
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        description="DIV2K training for CJSCC",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    # Dataset
    ap.add_argument("--data_root", type=str, default="./data_set")
    ap.add_argument("--batch_size",   type=int,   default=16)
    ap.add_argument("--num_workers",  type=int,   default=4)
    ap.add_argument("--seed",         type=int,   default=0)
    ap.add_argument("--save_dir",     type=str,   default="./checks/div2k")
    ...
    ap.add_argument("--early_stop_patience", type=int, default=20,
                    help="Stop training if validation PSNR does not improve.")

    # Channel conditions
    ap.add_argument("--equalize", action="store_true", default=False,
                    help="Enable MMSE equalization in channel model (default: off)")
    ...
    ap.add_argument("--rho_min",      type=parse_float_or_fraction, default="1/24")
    ...
    ap.add_argument("--ablation", type=str, default="full",
                    ...
                    help=(
                        "full    = LCDG with real CSI\n"
                        "no_lcdg = identity gates, no gating at all\n"
                        "no_csi  = LCDG architecture but CSI zeroed out"
                    ))
    ...
    # Data
    train_loader, val_loader, _ = make_div2k_kodak_loaders(
        root=args.data_root,
        crop_size=256,
    ...
    run_name  = (f"DIV2K_C{args.latent_channels}"
    ...
    if args.resume is not None:
        ...
        if os.path.isfile(resume_path):
            start_epoch, best_psnr = load_checkpoint(resume_path, model, opt, device)
            print(f"Resumed from {resume_path}  (epoch={start_epoch}, best_psnr={best_psnr:.3f})")
        else:
            print(f"[WARNING] --resume {args.resume} requested but {resume_path} not found. "
                  f"Starting from scratch.")

    print(f"\nDevice      : {device}")
    print(f"Ablation    : {args.ablation}  |  Params: {n_params:,}")
    print(f"Latent C    : {args.latent_channels}  |  Latent spatial: 64×64")
